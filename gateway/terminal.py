...
def publish_command(channel, event_type: str, target: str, payload: dict) -> str:
...
def wait_response(channel, correlation_id: str, timeout_seconds: int = 6):
    ...
    while time.time() < deadline:
        method, _, body = channel.basic_get(queue=QUEUE_NAMES["gateway"], auto_ack=False)
        if method:
            channel.basic_ack(delivery_tag=method.delivery_tag)
            envelope = json.loads(body.decode("utf-8"))
            if envelope.get("correlation_id") == correlation_id:
                payload = decrypt_for_component(envelope["encrypted_payload"], "gateway")
